...
namespace PersistentPlanet.Window
{
    public unsafe class Sdl2Window : Primitives.Platform.Window
    {
            ...
            bool threadedProcessing, KeyboardManager keyboardManager, MouseManager mouseManager)
        {
            ...
            {
                _window = Sdl2Native.SDL_CreateWindow(title, x, y, width, height, flags);
                _exists = true;
            }
        }
        ...
        public string Title { get => Sdl2Native.SDL_GetWindowTitle(_window); set => Sdl2Native.SDL_SetWindowTitle(_window, value); }
        public WindowState WindowState
        {
            get
            {
                SDL_WindowFlags flags = Sdl2Native.SDL_GetWindowFlags(_window);
            ...
            set
            {
                switch (value)
                {
                    case WindowState.Normal:
                        Sdl2Native.SDL_SetWindowFullscreen(_window, SDL_FullscreenMode.Windowed);
                        break;
                    case WindowState.FullScreen:
                        Sdl2Native.SDL_SetWindowFullscreen(_window, SDL_FullscreenMode.Fullscreen);
                        break;
                    case WindowState.Maximized:
                        Sdl2Native.SDL_MaximizeWindow(_window);
                        break;
                    case WindowState.Minimized:
                        Sdl2Native.SDL_MinimizeWindow(_window);
                        break;
                    case WindowState.BorderlessFullScreen:
                        Sdl2Native.SDL_SetWindowFullscreen(_window, SDL_FullscreenMode.FullScreenDesktop);
                        break;
                    default:
        ...
        public bool Visible
        {
            get => (Sdl2Native.SDL_GetWindowFlags(_window) & SDL_WindowFlags.Shown) != 0;
            set
            {
                if (value)
                {
                    Sdl2Native.SDL_ShowWindow(_window);
                }
                else
                {
                    Sdl2Native.SDL_HideWindow(_window);
                }
            }
        }
        ...
        public bool CursorVisible {
            get => Sdl2Native.SDL_ShowCursor(SDL_ShowCursorMode.Query) == SDL_ShowCursorMode.Enable;
            set => Sdl2Native.SDL_ShowCursor(value ? SDL_ShowCursorMode.Enable : SDL_ShowCursorMode.Disable);
        }
        ...
        {
            Closing?.Invoke();
            Sdl2Native.SDL_DestroyWindow(_window);
            _exists = false;
            Closed?.Invoke();
        }
        ...
        private void WindowOwnerRoutine(object state)
        {
            ...
            _window = Sdl2Native.SDL_CreateWindow(wp.Title, wp.X, wp.Y, wp.Width, wp.Height, wp.WindowFlags);
            wp.ResetEvent.Set();
        ...
        private void HandleKeyboardEvent(SDL_KeyboardEvent keyboardEvent)
        {
            ...
            {
                _keyboardManager.KeyDown(keyEvent.Key);
                KeyDown?.Invoke(keyEvent);
            }
            else
            {
                _keyboardManager.KeyUp(keyEvent.Key);
        ...
        {
            int x, y;
            Sdl2Native.SDL_GetWindowPosition(_window, &x, &y);
            return new Point(x, y);
        }

        private void SetWindowPosition(int x, int y)
        {
            Sdl2Native.SDL_SetWindowPosition(_window, x, y);
        }

        private Point GetWindowSize()
        {
            int w, h;
            Sdl2Native.SDL_GetWindowSize(_window, &w, &h);
            return new Point(w, h);
        }

        private void SetWindowSize(int width, int height)
        {
            Sdl2Native.SDL_SetWindowSize(_window, width, height);
        }
        ...
        private IntPtr GetUnderlyingWindowHandle()
        {
            ...
            Sdl2Native.SDL_GetWMWindowInfo(_window, &wmInfo);
        ...
        private bool GetWindowBordered() => (Sdl2Native.SDL_GetWindowFlags(_window) & SDL_WindowFlags.Borderless) == 0;
        private void SetWindowBordered(bool value) => Sdl2Native.SDL_SetWindowBordered(_window, value ? 1u : 0u);
